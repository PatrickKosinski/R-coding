...
shinyServer(function(input, output) {
        ...
        overall_bigrams <- readRDS("overall_bigrams.rds")
        overall_trigrams <- readRDS("overall_trigrams.rds")
        overall_unigrams <- readRDS("overall_unigrams.rds")
...
##################################################################################prediction########################################
        ...
        prediction_function <- function(chr_input_string, num_n, num_gamma, dt_overall_unigrams, dt_overall_bigrams, dt_overall_trigrams){
                ...
                if(sum(dt_get_observed_trigrams_probability$probability) >= 0.5 ){
                        ...
                        dt_get_observed_trigrams_probability <- dt_get_observed_trigrams_probability[1:num_n,]
                        # get the last words
                        ...
                        dt_get_trigrams_probability <- dt_get_trigrams_probability[1:num_n,]
                        # get the last words
...
####################################################################################################################################################################################################################        
        ...
        output$out1 <- renderText("This program completes a trigram based on a previously 
                                  included bigram. The prediction works via Katz-Back-Off (please see  'Further details'
                                  for explanations or click on the link below). Please type in at least two words, choose the number of predictions and confirm with run. The bar chart below shows 
                                  the most probable predictions based on the training data. 'Internally used string' 
                                  displays the string that is used as an input into the algorithm.")
        output$out2 <- renderText("The basic idea of Katz-Back-Off works as follows: As the user inputs a bigram (two words), the algorithm combines its vocabularly with the bigram forming a trigram (three words).
                                  Then, it is distinguished between observed and unobserved trigrams. We allocate the percentage
                                  of the occurance to the observed trigram substracted by a discount - the discount is supposed to 
                                  redistribute probability mass from observed to unobserved trigrams. In order to get the probabilities
                                  for unobserved trigrams we reduce the trigrams to bigrams (by deleting the first word), 
                                  and calculate the probabilities for bigrams. The procedure for observed bigrams is similar to
                                  the procedure to observed trigrams. For unobserved bigrams, we again reduce the bigram to 
                                  unigrams (single words). For these, we apply Maximum Likelihood estimation to get 
                                  the probabilities.")
...
####################################################################################################################################################################################################################        
        ...
        chr_input_string <- reactive({
                # clean inputstring up from "Sonderzeichen"
                help <- input$box1 
                if(sapply(strsplit(help, " "), length) == 1){
                        "WARNING: Please type in at least two words to execute a reasonable prediction. Barplot displays
                        three most frequent words."
                }else{
                        help <- gsub("[[:punct:]]|[[:digit:]]", " ", help)
                        help <- gsub("  ", " ", help)
                        help <- str_trim(paste(tail(strsplit(help, split=" ")[[1]],2)[1], tail(strsplit(help, split=" ")[[1]],2)[2], split=" "), "right")
                        help
                }
        })
        
        # input: how many predictions do I want to see?
        
        num_n <- reactive({
                input$slider1
        })
       
        # room for improvement: if there are not enough words typed in,
        # then i dont have to run the algorithm
        result <- reactive({
                
                prediction_function(chr_input_string(), num_n(), num_gamma = 0.1, dt_overall_unigrams = overall_unigrams, dt_overall_bigrams = overall_bigrams, dt_overall_trigrams = overall_trigrams)
        ...
        output$bar <- renderPlot(
                ggplot(result(), aes(x = reorder(ngram, - probability) , y = probability)) + geom_bar(stat = "identity", fill = "steelblue") + xlab("Predictions") + ylab("Probability") + theme(axis.text=element_text(size=20),  axis.title=element_text(size=14,face="bold"))
        )
